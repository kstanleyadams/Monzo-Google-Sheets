...
//Get basic account information
  ...
  for (var i = 0; i < account.length; i++) {
    var acc_id = account[i].id
    var created = account[i].created
  ...
  //Get balance and today's spend
    ...
    var total_balance = JSON.parse(result).total_balance;
  ...
  //Get the spreadsheet and ranges.
    ...
    var sheet = ss.getSheetByName("DataDump_" + i); 
    ...
    sheet.getRange('A1').setValue(balance / 100);
    sheet.getRange('B1').setValue((total_balance / 100) - (balance / 100));
  ...
  //Loop through each transaction to populate the spreadsheet, each value uses a try-catch loop to place values where a transaction does not contain some data.
    do{
      ...
      //Local Amount
      try{rangeValues[rowNum][3] = transactions[transactionNum].local_amount / 100;}catch(err){rangeValues[rowNum][3] = ""} 
      //Home Currenct Amount
      try{rangeValues[rowNum][4] = transactions[transactionNum].amount / 100;}catch(err){rangeValues[rowNum][4] = ""}  
      //Notes
  ...
  addKeysToAllTransactions(account.length)
...
function addKeysToAllTransactions(numberOfAccounts){
